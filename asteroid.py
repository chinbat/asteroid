    ...
    def draw(self, canvas):
        if self.animated and self.age < self.lifespan:
            canvas.draw_image(explosion_image, 
                            [EXPLOSION_CENTER[0] + self.age * EXPLOSION_SIZE[0],
                            EXPLOSION_CENTER[1]],
...
def click(pos):
    global started, rocks, score, lives, vel
    ...
    if (not started) and inwidth and inheight:
        ...
        vel = 2
def draw(canvas):
    global time, started, lives, score, my_ship, rock_group, missile_group, rocks
    ...
    canvas.draw_text("by Chinbaa", [640, 580], 22, "White")
    canvas.draw_text("Level:"+str(vel-2), [640, 480], 22, "White")
    ...
    if group_collide(rock_group, my_ship) != 0:
        ...
        if lives == 0:
            ...
            missile_group = set([])           
            soundtrack.pause()
    broken = group_group_collide(rock_group, missile_group)
    score += broken
    rocks -= broken
    #a_missile.update()
...
def rock_spawner():
    ...
    rock_vel = [random.random() * vel, random.random() * vel]
    rock_avel = random.random() * .2 - .1
    distance = dist(rock_pos, my_ship.get_position())
    if distance < 6 * my_ship.get_radius():
        rock_pos[0] = (rock_pos[0] + 6 * my_ship.get_radius())%width
        rock_pos[1] = (rock_pos[1] + 6 * my_ship.get_radius())%height
    if rocks < 20 and started:
...
timer1 = simplegui.create_timer(10000.0, level_upper)
